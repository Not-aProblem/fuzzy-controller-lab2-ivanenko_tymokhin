...
def preprocessing(input_lvs, output_lv):
    ...
    for lv in input_lvs:
        for term in lv["terms"].values():
            ...
            term["umf"] = getattr(t1_membership_functions, umf_type)(
                lv["U"], *umf_params
            )
            term["lmf"] = getattr(t1_membership_functions, lmf_type)(
                lv["U"], *lmf_params
            )
    ...
    for term in output_lv["terms"].values():
        ...
        term["range"] = (min(umf_params), max(umf_params))
        term["umf"] = getattr(t1_membership_functions, umf_type)(
            output_lv["U"], *umf_params
        )
        term["lmf"] = getattr(t1_membership_functions, lmf_type)(
            output_lv["U"], *lmf_params
        )
...
def implication(fuzzy_values, activated_rules, output_lv):
    ...
    for rule in activated_rules:
        ...
        tmp = t2_fuzzy_operators.t2_fuzzy_min(
            output_lv["terms"][consequent], min(lmf), min(umf)
        )
        result.append(tmp)
    return result
...
def process(input_lvs, output_lv, rule_base, crisp_values, cog):
    ...
    for term, fou in output_lv["terms"].items():
        sm = t2_similarity_measure.jaccard_measure(
            {"lmf": result_fuzzy_set[0], "umf": result_fuzzy_set[1]}, fou
        )
        word_result.append((term, sm))
    word_result = max(word_result, key=lambda item: item[1])

    return crisp_result, word_result[0], output_lv["U"], t1_fs, result_fuzzy_set
