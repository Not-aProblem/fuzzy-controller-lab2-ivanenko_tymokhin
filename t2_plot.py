...
def draw_t2_fuzzy_set(x, lmf, umf):
    ...
    ax.fill_between(x, lmf, umf, color="lightblue", edgecolor="b", alpha=0.5)
    plt.grid(True)
    st.pyplot(fig)
...
def draw_words_model(words_model):
    ...
    for number, word in enumerate(words_model["terms"], start=1):
        ax = fig.add_subplot(8, 4, number)

        x = words_model["U"]
        lmf, umf = words_model["terms"][word]["lmf"], words_model["terms"][word]["umf"]
